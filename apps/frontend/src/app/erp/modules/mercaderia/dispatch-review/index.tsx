...
import '../../../../../views/products/dispatch/style-items-print.css'
...
import {
  DispatchEditDrawer,
  useDispatchEditDrawer,
} from '@/views/products/dispatch/dispatch-edit-drawer'
import { DispatchItemsDrawer } from '@/views/products/dispatch/dispatch-items-drawer'
import {
  DivideDispatchDrawer,
  useDivideDispatchDrawer,
} from '@/views/products/dispatch/divide-dispatch.drawer'
import { GenerateGuidePopup } from '@/views/products/dispatch/generate-guide-popup'
import {
  ModifyDispatchedDrawer,
  useModifyDispatchDrawer,
} from '@/views/products/dispatch/modify-dispatched-drawer'
import { StatusTag } from '@/views/products/dispatch/status-tag'
import { useDispatchDetail } from '@/views/products/dispatch/use-dispatch-detail'
...
export const DispatchReviewPage = () => {
  const navigate = useNavigate()
  const [searchParams] = useSearchParams()

  const { data, isLoading, error, refetch } = useDispatchDetail(
    searchParams.get('dispatchId')
      ? Number(searchParams.get('dispatchId'))
      : null,
  )
  ...
  return (
    <>
      <div className="container mx-auto m-3">
        <div>
          <div
            className="my-2 inline-flex items-center gap-1 cursor-pointer"
            onClick={() => {
              navigate(PATHS.erp.modulos.mercaderia.despachos.deAlmacen)
            }}
          >
            <ArrowLeft /> Volver
          </div>
        ...
        {!searchParams.get('dispatchId') && (
          <div className="my-2 inline-flex items-center gap-1 cursor-pointer">
            <FiInfo /> No se ha seleccionado un despacho
          </div>
        )}
