...
export const DispatchStoreReviewPage = () => {
  // const { idDispatch, close } = useViewMoveDrawer()
  const [searchParams] = useSearchParams()

  const idDispatch = useMemo(
  ...
  return (
    <>
      ...
      {idDispatch && (
        <DrawerContent idDispatch={idDispatch} dispatchQuery={dispatchQuery} />
      )}
    </>
  )
}
    ...
    <div className="p-3 container mx-auto">
      <div>
        <div
          ...
          onClick={() => {
            navigate(PATHS.erp.modulos.mercaderia.despachos.entreTiendas)
          }}
        >
          <ArrowLeft /> Volver
        </div>
      ...
      <div className="flex justify-end font-bold">
        <p>Valor total del despacho: {fCurrency(dispatch?.totalValue ?? 0)}</p>
      </div>
    </div>
  )
}
