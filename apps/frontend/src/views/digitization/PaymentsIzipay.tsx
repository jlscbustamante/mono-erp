...
import {
  paymentFilesFiltersIziSt,
  paymentFilesIzipaySt,
...
export default function UploadFilePayments() {
  return (
...
const ActionsFilter = () => {
  ...
  const setFiles = useSetRecoilState(paymentFilesIzipaySt)
  const [filters, setFilters] = useRecoilState(paymentFilesFiltersIziSt)
  ...
  const applyFilters = async () => {
    try {
      ...
      const data = await sdk.filterPaymentFiles({
        ...filters,
        folder: [OpFilter.In, ...sdk.FoldersUpload.izipay],
      })
      setFiles(data)
    } catch (err: any) {
...
const ButtonUpload = () => {
  ...
  const openModalUpload = (type: PaymentUploadtype) => {
    setIsOpen(true)
    setTypeUpload(type)
  }
  const items = [
    {
      ...
      label: 'Izipay pos',
      onClick: () => {
        openModalUpload(PaymentUploadtype.Izipay)
      },
    },
    {
      key: 2,
      label: 'Izipay amex',
      onClick: () => {
        openModalUpload(PaymentUploadtype.IzipayAmex)
      },
    },
    {
      key: 3,
      label: 'Izipay dinner',
      onClick: () => {
        openModalUpload(PaymentUploadtype.IzipayDinner)
      },
    },
    {
      key: 4,
      label: 'Izipay mc',
      onClick: () => {
        openModalUpload(PaymentUploadtype.IzipayMc)
      },
    },
  ]
  ...
  const handleUpload = async () => {
    try {
      ...
      for (const _file of fileList) {
        ...
        try {
          if (typeUpload == PaymentUploadtype.IzipayAmex)
            await sdk.uploadIzipayAmex(file)
          else if (typeUpload == PaymentUploadtype.IzipayDinner)
            await sdk.uploadIzipayDinner(file)
          else if (typeUpload == PaymentUploadtype.Izipay)
            await sdk.uploadIzipay(file)
          else if (typeUpload == PaymentUploadtype.IzipayMc)
            await sdk.uploadIzipayMc(file)
        } catch (err: any) {
  ...
  const props: UploadProps = {
    ...
    onChange({ file, fileList: fileListFx }) {
      if (file.status !== 'uploading') {
        setFileList(fileListFx)
      }
    },
    beforeUpload: () => {
      return false
  ...
  return (
    <>
      ...
      <Modal
        ...
        centered={true}
        title={<p className="text-center">Agregar archivo : {typeUpload}</p>}
      >
        <div className="flex items-center justify-center my-4 flex-col">
          <Upload {...props} className="w-full flex flex-col items-center">
