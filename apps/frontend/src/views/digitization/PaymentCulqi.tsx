...
export default function PaymentCulqi() {
  return (
...
const ActionsFilter = () => {
  ...
  const setFiles = useSetRecoilState(paymentFilesCulqiSt)
  const [filters, setFilters] = useRecoilState(paymentFilesFiltersCulqiSt)
  ...
  const applyFilters = async () => {
    try {
      ...
      const data = await sdk.filterPaymentFiles({
        ...filters,
        folder: [OpFilter.In, ...sdk.FoldersUpload.culqi],
      })
      setFiles(data)
    } catch (err: any) {
  ...
  const items = [
    {
      ...
      label: 'Culqi pos',
      onClick: () => {
        openModalUpload(PaymentUploadtype.CulquiPos)
      },
    },
    {
      key: 2,
      label: 'Culqi online',
      onClick: () => {
        openModalUpload(PaymentUploadtype.Culqui)
      },
    },
    {
      key: 3,
      label: 'Culqi abonos',
      onClick: () => {
        openModalUpload(PaymentUploadtype.CulqiAbono)
      },
    },
  ]
  ...
  const handleUpload = async () => {
    try {
      ...
      for (const _file of fileList) {
        ...
        try {
          if (typeUpload == PaymentUploadtype.Culqui)
            await sdk.uploadCulquiOnline(file)
          else if (typeUpload == PaymentUploadtype.CulquiPos)
            await sdk.uploadCulquiPos(file)
          else if (typeUpload == PaymentUploadtype.CulqiAbono)
            await sdk.uploadCulqiAbono(file)
        } catch (err: any) {
  ...
  return (
    <>
      ...
      <Modal
        ...
        centered={true}
        // title={`Subir archivo : ${typeUpload}`}
...
const TableFiles = () => {
  const files = useRecoilValue(paymentFilesCulqiSt)
  const columns: ColumnsType<IPaymentFile> = [
    ...
    },
    {
      ...
      dataIndex: 'folder',
      key: 'folder',
      filters: sdk.FoldersUpload.culqi.map((el) => ({ text: el, value: el })),
