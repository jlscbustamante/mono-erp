...
import {
  defaultLiquidationForm,
  liquidationFormSt,
  setRequestChangeSt,
} from '@/data/requests/state'
import { Retention } from '@/data/requests/types'
import {
...
export const LiquidationForm = () => {
  ...
  const cashAccounts = useRecoilValue(cashAccountRequestSt).filter(
    (el) => el.cash_account_type.type_id === CashAccountTypeId.Liquidator,
  )
  const costCenters = useRecoilValue(costCentersSt)
  const [simpleForm, setSimpleForm] = useRecoilState(liquidationFormSt)
  ...
  const handlerCreate = async () => {
    try {
      ...
      setReloadChanges(1)
      setSimpleForm(defaultLiquidationForm)
    } catch (err: any) {
      toast.dismiss()
    ...
    <>
      <Form {...layout} style={{ maxWidth: 600 }}>
        <Form.Item label="Tipo">
          <Input placeholder="Liquidacion" disabled />
        </Form.Item>
