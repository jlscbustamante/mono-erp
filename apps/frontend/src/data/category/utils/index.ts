import { Filters, OpFilter } from '@/data/types/Filters'
import { safeAny } from '@/utils'

import { ICategory, ITypeCategory } from '../types'

export const transformFilterToValid = (
  filters: Filters<ITypeCategory>,
): Filters<ITypeCategory> => {
  const validFilters: Filters<ITypeCategory> = {}
  for (const [key, value] of Object.entries(filters)) {
    const tkey = key as keyof ITypeCategory
    const valuenotundefined: [OpFilter, ...safeAny[]] = value.filter(
      (v) => v !== undefined,
    ) as [OpFilter, ...safeAny[]]
    if (valuenotundefined.length <= 1) continue

    if (value[0] == OpFilter.Select)
      validFilters[tkey] = [OpFilter.Equal, valuenotundefined[1]]
    else if (value[0] == OpFilter.SelectIn)
      validFilters[tkey] = [OpFilter.In, ...valuenotundefined.slice(1)]
    else validFilters[tkey] = valuenotundefined
  }

  return validFilters
}
export const transformFilterToValidCategory = (
  filters: Filters<ICategory>,
): Filters<ICategory> => {
  const validFilters: Filters<ICategory> = {}
  for (const [key, value] of Object.entries(filters)) {
    const tkey = key as keyof ICategory
    const valuenotundefined: [OpFilter, ...safeAny[]] = value.filter(
      (v) => v !== undefined,
    ) as [OpFilter, ...safeAny[]]
    if (valuenotundefined.length <= 1) continue

    if (value[0] == OpFilter.Select)
      validFilters[tkey] = [OpFilter.Equal, valuenotundefined[1]]
    else if (value[0] == OpFilter.SelectIn)
      validFilters[tkey] = [OpFilter.In, ...valuenotundefined.slice(1)]
    else validFilters[tkey] = valuenotundefined
  }

  return validFilters
}
